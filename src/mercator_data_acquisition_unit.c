...
unsigned int mdau_create(uint32_t frame_size, FREQUENCY_t sampling_frequency, double vref){
	...
	spiData[0] = 0x01000000;
	spiData[1] = readFileValue(MMAP1_LOC "addr");
	spiData[2] = readFileValue(MMAP1_LOC "size");

	// Allocate and initialize memory
	if(prussdrv_init () != EXIT_SUCCESS){
		exit_state = EXIT_FAILURE;
		system("echo Failed prussdrv_init() >> /var/log/mercator/mercator.log");
	}
	if(prussdrv_open (PRU_EVTOUT_0) != EXIT_SUCCESS){
		exit_state = EXIT_FAILURE;
		system("echo Failed prussdrv_open() >> /var/log/mercator/mercator.log");
	}
	// Write the address and size into PRU0 Data RAM0. You can edit the value to
	// PRUSS0_PRU1_DATARAM if you wish to write to PRU1
	if(prussdrv_pru_write_memory(PRUSS0_PRU0_DATARAM, 0, spiData, 12) == -1){ // spi code
		exit_state = EXIT_FAILURE;
		system("echo Failed prussdrv_pru_write_memory on PRU0 >> /var/log/mercator/mercator.log");
	}
	if(prussdrv_pru_write_memory(PRUSS0_PRU1_DATARAM, 0, timerData, 8) == -1){ // sample clock
		exit_state = EXIT_FAILURE;
		system("echo Failed prussdrv_pru_write_memory on PRU1 >> /var/log/mercator/mercator.log");
	}
...
/* This method programs the PRU's and makes them execute their program. /root/mercator/PRUADC.bin and /root/mercator/PRUClock.bin should be present.
 * The PRU's fill the shared memory with 10-bit samples stored as uint_16. You should call mdau_wait_frame() to get the samples.
 */
void mdau_start_sampling(){
	...
	prussdrv_exec_program (ADC_PRU_NUM, "/root/mercator/PRUADC.bin");
	prussdrv_exec_program (CLK_PRU_NUM, "/root/mercator/PRUClock.bin");
}

/*	This method waits for the PRU's to finish their program started by mdau_start_sampling(). It reads the sampled values and allocates memory to store
 *	the samples as doubles. It will return a double* to these samples. To prevent memory leaks, the caller should free the samples once he is done with
 *	them. This is a blocking call.
 */
double* mdau_wait_frame(){
	if(VREF == 0){
		return NULL;
	}
